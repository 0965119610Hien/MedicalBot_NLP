...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('hellobacsi_crawler_3.log', encoding='utf-8'),
...
OUTPUT_DIR = Path("hellobacsi_data_3")
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    # Load cache
    cache_file = OUTPUT_DIR / "articles_3.json"
    crawled_articles = []
    crawled_urls = set()
    ...
    try:
        for cat_slug, cat_name in MAIN_CATEGORIES.items():
            ...
            # Save progress
            with open(OUTPUT_DIR / "discovered_urls_3.json", 'w', encoding='utf-8') as f:
    ...
    for i, (url, category) in enumerate(urls_list, 1):
        try:
            ...
            # Checkpoint
            if success % 50 == 0:
                save_checkpoint(crawled_articles, "articles_3.json")
    ...
    📁 Files:
       - {OUTPUT_DIR}/articles_3.json
       - {OUTPUT_DIR}/training_data_3.json
       - {OUTPUT_DIR}/training_data_3.jsonl
    """)
