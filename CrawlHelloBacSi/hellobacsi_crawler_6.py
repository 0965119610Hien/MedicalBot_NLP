...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('hellobacsi_crawler_6.log', encoding='utf-8'),
...
OUTPUT_DIR = Path("hellobacsi_data_6")
...
MAIN_CATEGORIES = {
    'thuoc': 'Thuốc và thực phẩm chức năng',
}
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    # Load cache
    cache_file = OUTPUT_DIR / "articles_6.json"
    crawled_articles = []
    crawled_urls = set()
    ...
    try:
        for cat_slug, cat_name in MAIN_CATEGORIES.items():
            ...
            # Save progress
            with open(OUTPUT_DIR / "discovered_urls_6.json", 'w', encoding='utf-8') as f:
    ...
    for i, (url, category) in enumerate(urls_list, 1):
        try:
            ...
            # Checkpoint
            if success % 50 == 0:
                save_checkpoint(crawled_articles, "articles_6.json")
    ...
    📁 Files:
       - {OUTPUT_DIR}/articles_6.json
       - {OUTPUT_DIR}/training_data_6.json
       - {OUTPUT_DIR}/training_data_6.jsonl
    """)
